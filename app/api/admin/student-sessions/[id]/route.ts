...
import { deleteStudentSession, updateStudentSessionStatus, writeAutomaticBackupSnapshot } from "@/lib/db";
import { z } from "zod";
...
export async function PATCH(request: Request, context: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    await updateStudentSessionStatus(Number(id), parsed.data.status);
...
export async function DELETE(_request: Request, context: { params: Promise<{ id: string }> }) {
  ...
  try {
    const { id } = await context.params;
    await deleteStudentSession(Number(id));
    await writeAutomaticBackupSnapshot();
    return NextResponse.json({ success: true });
  } catch (error) {
